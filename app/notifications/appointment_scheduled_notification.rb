class AppointmentScheduledNotification < ApplicationNotification
  deliver_by :action_cable, format: :to_websocket, channel: "NotificationChannel"
  deliver_by :email, mailer: "AppointmentsMailer", method: :appointment_scheduled, if: :email_notifications?
  ...
  def message
    "Appointment scheduled: #{ref_number} - #{appointment.start_datetime_string_in_zone(time_zone)} - #{language} - #{customer}"
  end
  ...
  def email_notifications?
    recipient&.notification_setting&.appointment_scheduled
  end
end
